...
(function($){
	...
	$.TextboxList = function(element, _options){
		...
		var afterInit = function(){
			if (options.endEditableBit) create('editable', null, {
		...
		this.onAdd = function(bit){
			...
			{
				index.push(uniqueValue(bit.getValue()));
				bits.push(bit);
		...
		this.onRemove = function(bit){
			...
			$(bits).each(function(i,k){
				if(k.value[0] === bit.value[0]) {
					bits.splice(i, 1);
					return false;
				}
			});
		...
		this.findBit = function(id) {
			for(i in bits) {
				if(id == bits[i].value[0]) {
					return bits[i];
				}
			}
		};
