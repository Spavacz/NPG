...
class CKFinder_Connector_CommandHandler_Thumbnail extends CKFinder_Connector_CommandHandler_CommandHandlerBase
{
	...
	public function sendResponse()
	{
		...
		// dodatkowe miniatury
		foreach ($_thumbnails->getAdditional() as $_thumb)
		{
			$addThumbFilePath = CKFinder_Connector_Utils_FileSystem::combinePaths($this->_currentFolder->getThumbsServerPath($_thumb['directory']), $fileName);
			if (!file_exists($addThumbFilePath))
			{
				if (!$this->createThumb($sourceFilePath, $addThumbFilePath, $_thumb['maxWidth'], $_thumb['maxHeight'], $_thumb['quality'], true, $_thumbnails->getBmpSupported()))
				{
					$this->_errorHandler->throwError(CKFINDER_CONNECTOR_ERROR_ACCESS_DENIED);
				}
			}
		...
		$rtime = isset($_SERVER["HTTP_IF_MODIFIED_SINCE"]) ? @strtotime($_SERVER["HTTP_IF_MODIFIED_SINCE"])
				: 0;
	 ...
	 * @access public
	 */
	public static function createThumb($sourceFile, $targetFile, $maxWidth,
		$maxHeight, $quality, $preserverAspectRatio, $bmpSupported = false)
	{
		...
		$sourceImageChannels = isset($sourceImageAttr["channels"]) ? $sourceImageAttr["channels"]
				: 3;
	 ...
	 * @access public
	 */
	public static function getAspectRatioSize($maxWidth, $maxHeight, $actualWidth,
		$actualHeight)
	{
