		...
		var g = {
			...
			addRow:	function(data){
				...
				if( p.controls ) {
					...
					for( var i in data.ctrl ) {
						...
						col.append('<a title="' + ctrlData.label + '" href="' + ctrlData.url + '" class="table-button ' + ctrlData.css + '" name="' + data.cols.id +
			...
			loadData: function(){
				if( p.url ) { // tylko jesli ustawiony jest url
					...
					// sprawdzam czy sa parametry
					var con = p.url.indexOf('?') == -1 ? '?' : '&';
					jQuery.getJSON( p.url + con + getData, function(result, textData) {
						if(result.success) {
